...
const sectionHandlers = sections.reduce((acc, section, i) => {
  ...
  section.maps.forEach((map) => {
    ...
    handlers.push({
      matches: (num) =>
        num >= sourceRangeStart && num < sourceRangeEnd,
      operation: (num) => {
        console.log(
          num,
          "->",
          num + (destinationRangeStart - sourceRangeStart)
        );
        return num + (destinationRangeStart - sourceRangeStart);
      },
      inRange: (start: number, stop: number) => {
...
export function part2() {
  ...
  for (
    let seedIndex = 0;
    seedIndex < seedRanges.length;
    seedIndex++
  ) {
    sectionHandlers.forEach((handlers, sectionIndex) => {
      // go through maps
      if (
        seedRanges[seedIndex].startAt &&
      ...
      handlers.forEach((handler, mapIndex) => {
        ...
        if (inRange.inRange === true) {
          ...
          if (
            ...
            seedRanges[seedIndex].start = handler.operation(
              handler.start
            );
            seedRanges[seedIndex].stop = handler.operation(
              seedRanges[seedIndex].stop
            );
