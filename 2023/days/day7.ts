...
const SCORE_KIND_STRENGTHS: ScoreKind[] = [
  "5oak",
  "4oak",
  "fh",
  "3oak",
...
const determineHandType = (hand: string[]) => {
  const cardCounts = CARD_STRENGTHS.map(
    (strength) => hand.filter((card) => card === strength).length
  );
...
function parseHandFromCards(hand: string[]): Hand {
  const kind = determineHandType(hand);
  const kindWeight =
    SCORE_KIND_STRENGTHS.length -
    SCORE_KIND_STRENGTHS.indexOf(kind);
  return {
    kind,
    kindWeight,
    hand,
    handWeight: hand.map((card: string) => {
...
function determineWinner(a: Hand, b: Hand) {
  let winner: Hand | null = null;
...
function part1() {
  ...
  return hands.reduce(
    (acc, item, index) => item.bet * (index + 1) + acc,
    0
  );
}
...
function determineHandTypePart2(hand: string[]) {
  if (hand.every((v) => v === "J")) {
    return "5oak";
  }
...
function parseHandFromCardsPart2(hand: string[]): Hand {
