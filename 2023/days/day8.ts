...
const [raw_instructions, _, ...lines] = getLines(
  "./day8.input.txt"
);
...
function runInstructions(
  ...
  while (
    condition(
      mutablePointers,
      totalInstructions % instructionsLength,
      totalInstructions
    )
  ) {
    for (let i = 0; i < loops; i++) {
      mutablePointers[i] =
        network[mutablePointers[i]][
          instructions[totalInstructions % instructionsLength]
        ];
    }
    totalInstructions += 1;
  }
...
function part1() {
  return runInstructions(
    ["AAA"],
    (pointers) => pointers[0] !== "ZZZ"
  ).totalInstructions;
}

function part2() {
  const nodesThatEndWithA = Object.keys(network).filter((key) =>
    key.endsWith("A")
  );

  return runInstructions(nodesThatEndWithA, (pointers) =>
    pointers.some((p) => !p.endsWith("Z"))
  ).totalInstructions;
}
...
function part2solve2() {
  ...
  const zPositions: Array<number> = [];
  const visitedPositions: Array<
    Array<{
      pointer: string;
      instructionIndex: number;
      totalIndex: number;
  ...
  const iterationsPerLoop = nodesThatEndWithA.reduce(
    (acc, startPos, iterationIndex) => {
      const totalIterationsBeforeLoop = runInstructions(
        [startPos],
        ([pointer], instructionIndex, totalIndex) => {
          if (pointer.endsWith("Z")) {
            zPositions.push(totalIndex);
          }
          if (
            visitedPositions[iterationIndex].find(
              (pos) =>
                pos.pointer === pointer &&
...
const longestLoop = info.iterationsPerLoop.reduce(
  (a, b) => Math.max(a, b),
  -Infinity
);

const longestLoopIndex =
  info.iterationsPerLoop.indexOf(longestLoop);
const longestLoopZPos =
  longestLoop - info.zPositions[longestLoopIndex];

const otherNums = info.iterationsPerLoop.filter(
  (v, i) => i !== longestLoopIndex
);

const otherOffsets = info.offsets.filter(
  (v, i) => i !== longestLoopIndex
);

const otherZIndices = info.zPositions.filter(
  (v, i) => i !== longestLoopIndex
);
...
while (finalPosition === null) {
  ...
  if (
    otherNums.every((checker, i) => {
      const currentRelativeLoopPosition = currentCheck % checker;
      const isAtZ =
        currentRelativeLoopPosition === otherZIndices[i];
      if (isAtZ && i > 0) {
        console.log(i, " at Z");
      }
      return isAtZ;
