...
function getStrNumbers(str) {
    const nums = [];
    Object.keys(numMap).forEach((strNum) => {
        let regex = new RegExp(strNum, "gi"), result, indices = [];
...
function getActualNumbers(str) {
    const nums = [];
    ["1", "2", "3", "4", "5", "6", "7", "8", "9"].forEach((strNum) => {
        let regex = new RegExp(strNum, "gi"), result, indices = [];
        while ((result = regex.exec(str))) {
            indices.push(result.index);
        }
...
export function part2() {
    const nums = lines.map((line) => {
        ...
        return [mapped[0], mapped.at(-1)];
    });
    return nums.reduce((acc, item) => {
        return (acc + parseInt(item[0].toString() + item[1].toString()));
    }, 0);
}
