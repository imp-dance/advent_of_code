import { getAdjacentIndices, getLines, matchAllWithIndex, } from "../utils/utils";
...
function checkIfPartNumber(value, startPos) {
    const adjacentIndices = getAdjacentIndices(startPos.col, startPos.row, value.toString().length);
    const adjacentCells = adjacentIndices.map(({ col, row }) => rows[row]?.[col]);
    return adjacentCells.some((cell) => ![undefined, "."].includes(cell));
}
function part1() {
    const partNumbers = [];
    lines.forEach((line, lineIndex) => {
        ...
        numbersOnLine.forEach((number) => {
            if (checkIfPartNumber(number.value, {
                row: lineIndex,
                col: number.index,
            })) {
...
function part2() {
    ...
    lines.forEach((line, lineIndex) => {
        ...
        starsOnLine.forEach((starIndex) => {
            const adjacentIndices = getAdjacentIndices(starIndex, lineIndex, 1);
            const adjacentCells = adjacentIndices.map(({ col, row }) => ({
                value: rows[row]?.[col],
                pos: { col, row },
            }));
            const adjacentCharacters = adjacentCells.filter((cell) => cell.value.match(/[0-9]/g));
            const adjacentNumbers = [
                ...new Set(adjacentCharacters.map((character) => {
                    const targetNumber = numbers[character.pos.row].find((number) => {
                        const col = number.index;
                        const length = number.value.toString().length;
                        const range = [col, col + length];
                        return (character.pos.col >= range[0] &&
